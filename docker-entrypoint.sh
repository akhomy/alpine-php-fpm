...
if [ "$USE_ONLY_CONFIGS" -eq "1" ]; then
    ...
    if [ -n "$PHP_FPM_PORT" ]; then
        sed -i 's@^listen.*@'"listen  = ${PHP_FPM_PORT}"'@' /etc/php5/php-fpm.conf
    fi
    ...
    if [ -n "$PHP_MAX_EXECUTION_TIME" ]; then
        sed -i 's@^max_execution_time.*@'"max_execution_time = ${PHP_MAX_EXECUTION_TIME}"'@' /etc/php5/php.ini
    fi
    ...
    if [ -n "$PHP_MAX_INPUT_TIME" ]; then
        sed -i 's@^max_input_time.*@'"max_input_time = ${PHP_MAX_INPUT_TIME}"'@' /etc/php5/php.ini
    fi
    ...
    if [ -n "$PHP_OPCACHE_ENABLE" ]; then
        sed -i 's@^opcache.enable.*@'"opcache\.enable = ${PHP_OPCACHE_ENABLE}"'@' /etc/php5/php.ini
    fi

    if [ -n "$PHP_OPCACHE_ENABLE_CLI" ]; then
        sed -i 's@^opcache.enable_cli.*@'"opcache\.enable_cli = ${PHP_OPCACHE_ENABLE_CLI}"'@' /etc/php5/conf.d/php.ini
    fi
    ...
    if [ -n "$PHP_UPLOAD_MAX_FILESIZE" ]; then
        sed -i 's@^upload_max_filesize.*@'"upload_max_filesize = ${PHP_UPLOAD_MAX_FILESIZE}"'@' /etc/php5/php.ini
    fi

    if [ -n "$PHP_ALLOW_URL_FOPEN" ]; then
        sed -i 's@^allow_url_fopen.*@'"allow_url_fopen = ${PHP_ALLOW_URL_FOPEN}"'@' /etc/php5/php.ini
    fi
    ...
    if [ "$PHP_XDEBUG_ENABLED" -eq "1" ]; then
        ...
        if [ -n "$PHP_XDEBUG_PORT" ]; then
            sed -i 's@^xdebug.remote_port.*@'"xdebug\.remote_port = ${PHP_XDEBUG_PORT}"'@' /etc/php5/conf.d/xdebug.ini
        fi
        if [ -n "$PHP_XDEBUG_IDEKEY" ]; then
            sed -i 's@^xdebug.idekey.*@'"xdebug\.idekey = ${PHP_XDEBUG_IDEKEY}"'@' /etc/php5/conf.d/xdebug.ini
        fi
        if [ -n "$PHP_XDEBUG_REMOTE_AUTOSTART" ]; then
            sed -i 's@^xdebug.remote_autostart.*@'"xdebug\.remote_autostart = ${PHP_XDEBUG_REMOTE_AUTOSTART}"'@' /etc/php5/conf.d/xdebug.ini
        fi
        if [ -n "$PHP_XDEBUG_REMOTE_CONNECT" ]; then
            sed -i 's@^xdebug.remote_connect_back.*@'"xdebug\.remote_connect_back = ${PHP_XDEBUG_REMOTE_CONNECT}"'@' /etc/php5/conf.d/xdebug.ini
        fi
